...
(defn settings-slider-at [pos]
  (some (fn [index]
          (let [slider-settings-circle (settings-circle
                                        c/settings-sliders-page)
                left (-> slider-settings-circle
                         (update :radius
                                 (partial * c/settings-slider-radius))
                         (update :x
...
(defn saved-formbar-scroll-circles []
  (let [center-circle (settings-circle c/settings-saved-formbars-page)
        center-radius (:radius center-circle)
        scroll-bar-radius (* center-radius
                             c/settings-saved-formbars-scroll-radius)
        ...
        scroll-top-y (+ (:y center-circle)
                        ...
                        scroll-bar-radius)
        saved-formbar-box-height
        (+ (* 2 (* c/settings-saved-formbar-radius center-radius)
      ...
      (let [center-radius (:radius saved-formbar-circle)
            ...
            (+ (* 2 formbar-radius
               ...
               (* center-radius
                  ...
                  (inc c/settings-saved-formbars-box-height)))
            formbar-zone-corner
            (-> saved-formbar-circle
                (select-keys [:x :y])
                (update :x (partial + (- (* c/settings-saved-formbars-box-x
                                            center-radius)
                                         (* 0.5 saved-formbar-box-width))))
                (update :y (partial + (* center-radius
                                         c/settings-saved-formbars-box-y))))]
        (when (in-rect? [formbar-zone-corner
          ...
          (int
           (u/clamp
            0
            (dec c/settings-saved-formbars-box-height)
   ...
   {:enter
    ...
    (fn []
      (let [current-size (app-size)
        ...
        (when dropdown
          (let [{:keys [x y radius]} (settings-circle
                                      c/settings-project-selector-page)
          ...
          (let [[slider-name slider-key] (nth c/settings-sliders slider-index)
            ...
            (doseq [p [right left]]
              (draw-circle (assoc p
                           ...
                           (:background (color-scheme))
                           :background))
            (draw-circle (assoc (tween-points left
                                              right
                                              (global-attr slider-key))
                                :radius
                                (* center-radius
                                   c/settings-slider-radius
      ...
      (let [center-circle (settings-circle c/settings-formbar-tools-page)
        ...
        (draw-text "Tools"
                   (-> center-circle
                       (update :y
                               (partial + (* center-radius
                                             c/settings-formbar-tool-text-y))))
      ...
      (let [center-circle (settings-circle c/settings-saved-formbars-page)
        ...
        (draw-text
         ...
         (-> center-circle
             (update :y
                     (partial + (* center-radius
                                   c/settings-saved-formbars-text-y))))
        ...
        (let [formbar-radius (* center-radius c/settings-saved-formbar-radius)
              ...
              (-> center-circle
                  (select-keys [:x :y])
                  (update :x
                          (partial + (- (* c/settings-saved-formbars-box-x
                                           center-radius)
                                        (* 0.5 saved-formbar-box-width))))
                  (update :y
                          (partial + (* center-radius
         ...
         ; Formbar area background
          (let [corner-radius (* (inc c/saved-formbar-spacing)
                                 center-radius
                                 c/saved-formbar-zone-corner-radius)]
            (doseq [y [corner-radius
              ...
              (draw-circle (-> formbar-zone-corner
                               (assoc :radius corner-radius)
                               (update :x (partial + (- saved-formbar-box-width
                                                        corner-radius)))
                               (update :y (partial + y)))
                           (:background (color-scheme))
                           :background))
         ...
         ; Saved formbars
          (let [current-saved-formbar-contents (saved-formbar-contents)
                formbar-index-offset @saved-formbar-scroll-pos
                hovered-formbar-index (saved-formbar-index-at mouse)]
            (doseq [formbar-index
                    (map (partial + formbar-index-offset)
                         (range (min (- (count current-saved-formbar-contents)
                                        formbar-index-offset)
                                     c/settings-saved-formbars-box-height)))]
              (let [adjusted-formbar-index (- formbar-index
                                              formbar-index-offset)
                    formbar-forms (nth current-saved-formbar-contents
                                       formbar-index)
                    formbar-oversized (> (count formbar-forms)
                                         c/settings-saved-formbars-box-width)
                    draw-bar
                    (fn [scale color layer]
            ...
            (when (and (:down? mouse)
                       (or (= :formbar (:down-zone mouse))
                           (and (= :saved-formbar (:down-zone mouse))
                                (< (+ (saved-formbar-index-at
                                       (:down-pos mouse))
                                      @saved-formbar-scroll-pos)
                                   (count current-saved-formbar-contents)))))
              (let [insertion-index
                    (min (saved-formbar-insertion-index-at mouse)
                         (count current-saved-formbar-contents))]
      ...
      (let [formbar-path (formbar-path-at mouse)]
        (when formbar-path
          (let [current-formbar-arrangement (formbar-arrangement)
                hovered-formbar (get-in current-formbar-arrangement
                                        formbar-path)
    ...
    (fn [delta mouse]
      (when (:down? mouse)
        (when (= (:down-zone mouse) :settings-slider)
          (let [settings-circle (settings-circle c/settings-sliders-page)
                ...
                adjusted-x-off (/ x-off (* (:radius settings-circle)
                                           c/settings-slider-width))]
            ...
            (let [raw-angle
                  ...
                  (some (fn [angle]
                          (when (< (min (Math/abs (- angle raw-angle))
                            ...
                            angle))
                        (mapv (partial * TAU)
                              (u/prop-range c/scroll-angle-snap-positions
                                            true)))]
              (or snap-angle
                  raw-angle))))))
      ...
      (when (and (:down? mouse)
                 (= (:down-zone mouse)
                    :saved-formbar-scroll))
        (let [[scroll-top-y scroll-bottom-y]
              (mapv :y (saved-formbar-scroll-circles))
              scroll-pos
              (u/clamp (u/map-range scroll-top-y
                                    scroll-bottom-y
                                    0
                                    1
                                    (:y mouse)))
    ...
    (fn [mouse mouse-zone]
      (cond
        ...
        (case (:down-zone mouse)
          ...
          :saved-formbar
          (let [adjusted-formbar-index (+ (saved-formbar-index-at
                                           (:down-pos mouse))
                                          @saved-formbar-scroll-pos)]
      ...
      (if (:dragging? mouse)
        (cond
          ...
          (let [formbar-placement-path
                (formbar-insertion-path-at mouse)
                saved-formbar-insertion-index
          ...
          (if (= mouse-zone :saved-formbar)
            ...
            (when (not (in-discard-corner? mouse))
              (let [formbar-placement-path (formbar-insertion-path-at mouse)
                    saved-formbars (saved-formbar-contents)]
                (when formbar-placement-path
                  (let [selected-saved-formbar-index
                        (+ @saved-formbar-scroll-pos
                           (saved-formbar-index-at (:down-pos mouse)))
                        saved-formbar
                        (nth saved-formbars selected-saved-formbar-index)]
                    (add-project-formbar-at formbar-placement-path)
                    (doseq [index (range (count saved-formbar))]
                      (let [form (nth saved-formbar index)]
                        (add-project-formbar-form-at form
                                                     formbar-placement-path
                                                     index)))))))))
        (case (:down-zone mouse)
          :back-icon
